...
CREATE OR REPLACE FUNCTION set_new_supply_for_recipe(pid_recipe INT, pid_material INT, pid_measurement INT, punit_cost REAL, ptotal_cost REAL)
RETURNS VOID AS $$
BEGIN
    INSERT INTO supply_for_recipe(unit_cost,total_cost,id_measurement, id_recipe, id_material) 
    VALUES (punit_cost, ptotal_cost, pid_measurement, pid_recipe, pid_material);
END; $$
LANGUAGE plpgsql;
