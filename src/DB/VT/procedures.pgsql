...
RETURNS TABLE(Id integer,Apellidos character varying,Nombre character varying,Fecha date,Total real,Estado integer,Cantidad integer,Precio_Unitario real)
 AS $$
    DECLARE
        
    BEGIN
         RETURN QUERY
             select order_production.order_production_id,customer.last_name,customer.first_name,order_prod_date,order_production.total,order_state,quantity,unit_price 
             from order_production 
             inner join order_production_detail on order_production_detail.order_production_id=order_production.order_production_id
    ...
    END;
$$ LANGUAGE plpgsql;
CREATE OR REPLACE FUNCTION getOrderByDate(pOrderDate date) 
RETURNS TABLE(Id integer,Apellidos character varying,Nombre character varying,Fecha date,Total real,Estado integer,Cantidad integer,Precio_Unitario real)
 AS $$
    DECLARE
...
CREATE OR REPLACE PROCEDURE insertIntoLog(pid_sale integer,porder_production_id integer,pupdate_time date,pstatus integer) 
 AS $$
    DECLARE
        
    BEGIN
         INSERT INTO product_log(id_sale,order_production,update_time,status)
         values(pid_sale,porder_production_id,pupdate_time,pstatus);
    END;
$$ LANGUAGE plpgsql;    
--7
